...
${CC} -nostdlib -T ${LINKER_SCRIPT} -g -fno-tree-loop-distribute-patterns -mtext-section-literals -mlongcalls -fno-builtin -fno-stack-protector -fno-pic -fno-pie -Wall -Wextra  -std=c99 -pedantic -ffreestanding \
    -mabi=call0 \
    ${INCLUDES} \
    -o ${KERNEL_ELF} \
    ${ARCH_DIR}/boot/boot.S ${ARCH_DIR}/src/vector.S ${ARCH_DIR}/src/smp.S ${SRC_DIR}/kernel/kernel.c ${SRC_DIR}/memory_manager/memory.c ${SRC_DIR}/interrupts/interrupts.c ${SRC_DIR}/watchdog/watchdog.c ${SRC_DIR}/smp/smp.c ${SRC_DIR}/scheduler/scheduler.c ${SRC_DIR}/scheduler/spinlock.c ${SRC_DIR}/scheduler/mutex.c ${SRC_DIR}/scheduler/semaphore.c ${SRC_DIR}/scheduler/cond.c ${SRC_DIR}/scheduler/event_group.c ${SRC_DIR}/utils/utils.c ${SRC_DIR}/syscall/syscall.c ${ARCH_DIR}/drivers/esp32_gpio/gpio.c ${ARCH_DIR}/drivers/esp32_led/led.c ${ARCH_DIR}/drivers/esp32_dma/dma.c ${ARCH_DIR}/drivers/esp32_spi/spi.c ${ARCH_DIR}/drivers/esp32_sdcard/sdcard.c ${SRC_DIR}/file_system/ramfs/ramfs.c ${SRC_DIR}/file_system/devfs/devfs.c ${SRC_DIR}/file_system/fat32/fat32.c ${SRC_DIR}/loader/elf_loader.c \
    -lgcc
